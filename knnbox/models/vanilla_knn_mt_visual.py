...
class VanillaKNNMTVisual(TransformerModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--knn-xlingual-map", type=str, default=None,
                            help="Apply xlingual mapping before storing keys")
...
class VanillaKNNMTVisualDecoder(TransformerDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.knn_xlingual_map = args.knn_xlingual_map
        ...
        if args.knn_mode == "build_datastore":
            ...
            # Load cross-lingual map
            if self.knn_xlingual_map is not None:   
                assert torch.cuda.is_available()
                self.xlingual_map = torch.from_numpy(np.load(self.knn_xlingual_map)).cuda()
    ...
    def forward(
        ...
        **kwargs, ## we need get dynamic k from kwargs, so add it here 
        ...
        if self.args.knn_mode == "build_datastore":
            ...
            if self.knn_xlingual_map:
                keys = keys@self.xlingual_map
