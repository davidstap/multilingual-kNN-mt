...
class VanillaKNNMT(TransformerModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--knn-return-values", default=None,
                            help="choose return values")
        parser.add_argument("--knn-xlingual-map", type=str, default=None,
                            help="Path to cross-lingual mapping, applied to keys before datastore search")
...
class VanillaKNNMTDecoder(TransformerDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.save_knn_data = False
        self.return_list = ["vals", "distances"]
        if args.knn_return_values == "all":
            self.save_knn_data = True
            self.return_list.extend(["sentence_ids", "src_lang_tags", "tgt_lang_tags", "token_positions"])
        
        if args.knn_mode == "build_datastore":
            if "datastore" not in global_vars():
                # regist the data`store as a global variable if not exist,
            ...
            load_list = ["vals"]
            if args.knn_return_values == "all":
                load_list.extend(["sentence_ids", "src_lang_tags", "tgt_lang_tags", "token_positions"])
            # when inference, we don't load the keys, use its faiss index is enough
            self.datastore = Datastore.load(args.knn_datastore_path, load_list=load_list)
            self.datastore.load_faiss_index("keys")
            self.retriever = Retriever(datastore=self.datastore, k=args.knn_k, save_knn_data=self.save_knn_data, knn_xlingual_map=args.knn_xlingual_map)
            self.combiner = Combiner(lambda_=args.knn_lambda, temperature=args.knn_temperature, probability_dim=len(dictionary), save_top_k=self.save_knn_data)
    ...
    def forward(
        ...
        if self.args.knn_mode == "build_datastore":
            ...
            self.retriever.retrieve(x, return_list=self.return_list)
