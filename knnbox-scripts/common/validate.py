    ...
    for subset in args.valid_subset.split(","):
        ...
        for i, sample in enumerate(progress):
            ...
            if knn_type in ["vanilla_knn_mt", "adaptive_knn_mt", "greedy_merge_knn_mt", "kernel_smoothed_knn_mt", "plac_knn_mt", "robust_knn_mt"]:
                ...
                # get language tags for source sentences
                src_lang_tags = []

                # print(sample["target"].cpu().numpy())

                # quit()

                for idx, src_tok in enumerate(sample["net_input"]["src_tokens"].cpu().numpy()):
                    src_lang_tag = src_tok[0]
                    src_lang_tags += [src_lang_tag]*target_len[idx]
                src_lang_tags = np.array(src_lang_tags, dtype=int)
                # get language tags for target sentences
                tgt_lang_tags = []
                for idx, tgt_tok in enumerate(sample["target"].cpu().numpy()):
                    tgt_lang_tag = tgt_tok[0]
                    tgt_lang_tags += [tgt_lang_tag]*target_len[idx]
                tgt_lang_tags = np.array(tgt_lang_tags, dtype=int)
                target_len = target_len.cpu().numpy()
                # lengths should be the same
                assert all(
                    len(x) == len(sentence_ids)
                    for x in [sentence_ids, token_positions, src_lang_tags, tgt_lang_tags]
                )
                # add them to datastore
                datastore["sentence_ids"].add(sentence_ids)
                datastore["token_positions"].add(token_positions)
                datastore["src_lang_tags"].add(src_lang_tags)
                datastore["tgt_lang_tags"].add(tgt_lang_tags)
