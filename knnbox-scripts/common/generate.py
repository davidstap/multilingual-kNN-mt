...
import numpy as np
from knnbox.common_utils.function import save_knn_vals_to_disk
import torch
...
def _main(args, override_args, output_file):
    ...
    # Save knn stats to pkl file
    if args.knn_return_values == "all":
        assert len(models) == 1 # we do not support ensembles for now
        logger.info(f"Saving kNN-MT stats to text file {args.results_path}/stats.pickle")
        save_knn_vals_to_disk(
            src_dict=src_dict,
            tgt_dict=tgt_dict,
            retriever=models[0].decoder.retriever,
            combiner=models[0].decoder.combiner,
            file_name=f"{args.results_path}/stats.pickle"
        )
